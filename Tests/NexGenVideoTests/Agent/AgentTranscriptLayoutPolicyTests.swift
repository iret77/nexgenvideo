...
struct AgentTranscriptLayoutPolicyTests {
    ...
    @Test func spendApprovalKeepsValidProviderAndModelChoicesInTheCard() throws {
        ...
        #expect(card.components(separatedBy: "NativeChoicePicker(").count - 1 == 2)
        #expect(card.contains("label: \"Provider\""))
        #expect(card.contains("selection: providerSelection"))
        #expect(card.contains("label: \"Model\""))
        #expect(card.contains("selection: $selectedOptionId"))
