...
struct PatternProviderTests {
    ...
    func recommendRanksWhatIsScored() throws {
        ...
        let invalid = try #require(coverage["invalid"] as? [String])
        #expect(scored.contains("wong-kar-wai-doyle-neon-dream"))
        #expect(!unscored.contains("wong-kar-wai-doyle-neon-dream"))
        #expect(invalid.isEmpty)
        #expect(coverage["total"] as? Int == scored.count + unscored.count + invalid.count)
    ...
    func getById() throws {
        ...
        let camera = try #require(obj["camera"] as? [String: Any])
        let lighting = try #require(obj["lighting"] as? [String: Any])
        let color = try #require(obj["color"] as? [String: Any])
        let craft = try #require(obj["craft_signature"] as? [[String: Any]])
        #expect(camera["basis"] as? String == "inferred")
        #expect(lighting["sources"] as? [String] != nil)
        #expect(color["sources"] as? [String] != nil)
        #expect(craft.allSatisfy { ($0["pipeline_levers"] as? [String])?.isEmpty == false })
        #expect(obj["approximation_basis"] == nil)
        #expect(obj["camera_vocabulary"] == nil)
        #expect(obj["lighting_signature"] == nil)
