...
- **Gate:** after writing, summarize briefly and ask the user for
  explicit approval via AskUserQuestion ("Approve the brief, change
...
### 2. Mandatory question catalog (always ask all of them, via AskUserQuestion)

#### Batch 1 — 4 questions (one AskUserQuestion call)
...
#### Batch 1a — visual medium (own AskUserQuestion call right after Batch 1)
...
6. **Concrete style / reference vocabulary** — its OWN AskUserQuestion
...
#### Technical questions (own AskUserQuestion calls after Batch 1b)
...
14. **Stems provider — status & action**
    ...
    - **Only if the user actively wants to switch** (rare — usually
      ...
      AskUserQuestion with the note that switching re-runs the analysis
      (`run_phase(project_dir, "analysis")`). The default is always
      "stay with the current provider."

15. **Final resolution** (mandatory). `AskUserQuestion`:
...
16. **Preview pass** (mandatory). First show the user the budget picture,
    ...
    c) **AskUserQuestion** with the recommendation + facts as rationale:
...
18. **Director pattern** (optional but recommended)
    ...
    b) **AskUserQuestion** with the suggestions — for each pattern show
