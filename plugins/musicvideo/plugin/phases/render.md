...
- Call `get_render_manifest(project_dir, "<phase>")`. Reconcile per
  ...
  - Pending or missing shots → `AskUserQuestion`: "Continue in the
...
Before every R run, `AskUserQuestion`:
...
1. **Never unilaterally** — explicit `AskUserQuestion` with a
