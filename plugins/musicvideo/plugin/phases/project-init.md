...
- **STRICTLY FORBIDDEN:** an AskUserQuestion with invented project-name
...
- Example: "Way In Life" → `way_in_life`, "Straße nach Osten" →
  `strasse_nach_osten`.
- Show the proposed slug and **only then** ask via `AskUserQuestion`
  whether to adopt it or choose differently (options: "Accept slug" /
  "Different slug" — under "Other" the user can type their own slug).
  This AskUserQuestion is **only** about the slug form of the title
...
slug under the project home): `AskUserQuestion` "abort / continue working
...
- Project slug already exists → `AskUserQuestion` "abort / continue
