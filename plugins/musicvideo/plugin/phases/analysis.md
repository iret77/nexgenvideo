...
- **Gate:** after writing, give a summary (BPM, key, section labels,
  anomalies, which stages ran) and request approval via AskUserQuestion
...
- **Lyrics or reference images missing** → **AskUserQuestion** before the
...
- Does `analysis/<song>.json` already contain a top-level key
  ...
  `overall_character`? → AskUserQuestion: "An interpretation already
...
2. Ask the user an AskUserQuestion with the three plausible options:
...
- Preflight: lyrics/refs missing → AskUserQuestion before any expensive
