...
public protocol PatternProviding: Sendable {
    ...
    /// on success. Valid authored profiles rank immediately; coverage names unscored and invalid content.
    /// JSON is the currency so the engine stays agnostic of the pack's fit schema. Throws only on a
    /// genuine configuration error (missing policy, no project input).
    func recommend(briefJSON: Data, optionsJSON: Data) throws -> Data

    /// The full pattern for `id` as JSON, including per-value provenance, citable craft techniques,
