...
public struct ProductionKnowledgeArchive34: Codable, Sendable {
    ...
    public struct Record: Codable, Sendable {
        ...
        public let intendedConsumers: [String]
    ...
    public func readTechnique(_ id: String, mediumID: String? = nil) throws -> [Read] {
        ...
        if id == "C" {
            guard let mediumID, let medium = records.first(where: { $0.id == mediumID }),
                  medium.kind == "medium-row" else {
                throw ProductionKnowledgeErrorV1.invalidValue(path: "mediumID", reason: "choose exactly one medium-row from the index")
            }
            ids = ["style-control-229b9a58a150-u104", mediumID,
                   "style-control-229b9a58a150-u12271", "style-control-229b9a58a150-u13174",
                   "style-control-229b9a58a150-u18054"]
            if medium.title.lowercased().contains("sumi") { ids.append("style-control-229b9a58a150-u15328") }
        } else if mediumID != nil {
            throw ProductionKnowledgeErrorV1.invalidValue(path: "mediumID", reason: "only technique C selects a medium row")
        }
    ...
    func validate() throws {
        ...
        for record in records {
            ...
            } else if record.kind == "unit" || record.kind == "medium-row" {
...
extension EngineProductionKnowledgeResourcesV1 {
    private static let archive34: Result<ProductionKnowledgeArchive34, Error> = Result {
