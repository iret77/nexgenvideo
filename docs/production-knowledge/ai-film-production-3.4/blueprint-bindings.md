> 3.4 source update: every recipe now has an individual source heading. `blueprints.json` preserves all 42 stable recipe IDs and extracts every complete Verify clause, including its explicit `[edit]` marker. Markers are postfix to the preceding question; `verifyText` remains intact for audit. Each criterion retains evidence requirements. Intake questions are grouped into the single permitted intake message; unanswered craft choices remain labelled ASSUMED.

# Director and DoP blueprints: NGV application contract

Status: 3.4 adaptation specification. Baseline #492 is implemented; new bindings require explicit migration. The complete 31 director recipes and 11 DoP signatures are materialized in `blueprints.json`; the selection index, mood table, constraint filter, aliases, documented partnerships, experimental cross-pairings and clashes are preserved in `chapters/director-recipes.json` and the readable handbook. These are production recipes, not a list of director names or five generic camera moods.
...
`dimensions` retains source labels, including combined `Light/Color`; do not invent a missing dimension. `completeRecipeMarkdown` is authoritative for nuance such as variants and AI notes that sit between named labels. Parsing the fields is not permission to drop that text. Compile deliberately needed recipe controls through the chosen project technique: A uses its STYLE tokens and beat-local controls; B distributes them into necessary blocks; C uses its single seven-layer Master Style Block. Model containers such as H3 remain binding. Do not fill every IR axis or repeat recipe values as boilerplate.
...
Recipe checks are review criteria, not imaginary machine observations. Structural checks can prove IDs, bindings and scope, but cannot prove lighting or emotion. The reviewer must actually see/hear the evidence or record attributed director answers. Missing evidence remains `not_observed`. Explicit source `[edit]` clauses are assembly-only and never trigger a generation reroll. Other source-mandated aesthetic checks are retained; technical hard rejects are evaluated before emotional ranking, then the winning production take's local fault takes the repair route. Do not implement “Verify = automatically reroll everything.”
