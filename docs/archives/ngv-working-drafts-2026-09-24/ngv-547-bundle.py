...
root=Path('<local NexGenVideo checkout>.worktrees/issue-547')
...
def record(id,s,text,kind,title=None,start=None,end=None):
 ...
 records.append(dict(id=id,sectionID=s['id'],title=title or s['title'],kind=kind,text=text,sha256=sha(text),source=s['source'],startCharacter=start,endCharacter=end,applicationContract=c['id'],consumers=c.get('runtimeConsumers',[]),disposition='dated-evidence-not-executable' if evidence else 'retrievable-guidance-under-current-contract'))
...
(src/'runtime-ledger.json').write_text(json.dumps(ledger,ensure_ascii=False,indent=2)+'\n')
