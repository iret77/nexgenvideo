...
try{
 ...
 const {targetId}=await call('Target.createTarget',{url:'file:///tmp/ngv-controls-host.html'});
 ...
 await evalJS("document.getElementById('ngv-desk').ngvTest.fixture('Leer','Generic');document.getElementById('ngv-desk').ngvTest.action('studio:settings');document.getElementById('ngv-desk').ngvTest.action('studio:settings-models');document.getElementById('ngv-desk').ngvTest.action('studio:model-research')");
 await call('Page.bringToFront',{},topSession);await sleep(300);
 console.log(JSON.stringify(await evalJS("(()=>{const r=document.getElementById('ngv-desk'),d=r.querySelector('#nd-task'),b=d.getBoundingClientRect();return {ready:!!r.ngvTest,width:b.width,height:b.height,overflow:d.scrollWidth>d.clientWidth,checkGaps:[...d.querySelectorAll('.finish-check')].map(l=>l.querySelector('span').getBoundingClientRect().left-l.querySelector('input').getBoundingClientRect().right),axes:[...d.querySelectorAll('.studio-task-head,.studio-task-content,.studio-task-options')].map(e=>({tag:e.tagName,left:e.getBoundingClientRect().left,right:e.getBoundingClientRect().right}))}})()")));
 const shot=await call('Page.captureScreenshot',{format:'png',captureBeyondViewport:false},topSession);await writeFile(here+'/review/control-spacing-2026-09-19/inline-model-research.png',Buffer.from(shot.data,'base64'));
