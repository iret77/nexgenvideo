...
try {
 ...
 for(const width of [1048,390]){
  ...
  for(const command of ['model-research','music']){
   await run(`a.fixture('Schnitt');const s=st();s.studio.uiScale=130;a.restore(s);a.action('studio:${command}');`);
   await capture(width+'-130-'+command);await inspect(width+'-130-'+command,'#nd-task');
  }
 }
 for(const width of [1440,1048,768,390])for(const scale of [100,130]){
  await call('Emulation.setDeviceMetricsOverride',{width,height:1100,deviceScaleFactor:1,mobile:false},session);
  await run(`a.fixture('Medien');const s=st();s.studio.uiScale=${scale};a.restore(s);`);
  await capture('media-'+width+'-'+scale,'.library-filters');await inspect('media-'+width+'-'+scale);
  await test('Media labels fit without overlaps '+width+'/'+scale,`const l=q('.library-ai'),range=document.createRange();range.selectNodeContents(l.querySelector('span'));const b=l.getBoundingClientRect(),text=range.getBoundingClientRect(),siblings=[...l.parentElement.children].filter(x=>x!==l&&x.offsetWidth&&x.offsetHeight);return text.right<=b.right+1&&text.left>=b.left&&siblings.every(x=>{const s=x.getBoundingClientRect();return Math.min(s.right,b.right)-Math.max(s.left,b.left)<1||Math.min(s.bottom,b.bottom)-Math.max(s.top,b.top)<1});`);
 }
 ...
 await writeFile(out+'/final-visual-checks.json',JSON.stringify({measurements,results},null,2));
 console.log(JSON.stringify(results));if(results.some(x=>!x.pass))process.exitCode=1;
