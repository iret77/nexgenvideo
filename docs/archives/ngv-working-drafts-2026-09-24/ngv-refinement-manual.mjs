...
const chrome=spawn('google-chrome',['--user-data-dir=/tmp/ngv-manual-'+process.pid,'--headless=new','--no-sandbox','--disable-gpu','--disable-dev-shm-usage','--no-first-run','--remote-debugging-pipe','--window-size=1280,1000','about:blank'],{stdio:['ignore','ignore','ignore','pipe','pipe']});
...
console.log('READY');
await mkdir('/tmp/ngv-refinement-manual',{recursive:true});
const log=[];
for await(const line of createInterface({input:process.stdin})){
 ...
 else if(c.type==='screenshot'){const s=await call('Page.captureScreenshot',{format:'png',captureBeyondViewport:false});await writeFile('/tmp/ngv-refinement-manual/'+c.name+'.png',Buffer.from(s.data,'base64'));result='/tmp/ngv-refinement-manual/'+c.name+'.png';}
 ...
 log.push({command:c,result});await writeFile('/tmp/ngv-refinement-manual/trace.json',JSON.stringify(log,null,2));console.log(JSON.stringify(result));
