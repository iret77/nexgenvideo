...
try{
 ...
 const {targetId}=await call('Target.createTarget',{url:'file:///tmp/ngv-settings-host.html'});
 ...
 await evalJS("document.getElementById('ngv-desk').ngvTest.fixture('Leer','Generic');document.getElementById('ngv-desk').ngvTest.action('studio:settings');document.getElementById('ngv-desk').ngvTest.action('studio:settings-general')");
 await call('Page.bringToFront',{},topSession);await sleep(300);
 console.log(JSON.stringify(await evalJS("(()=>{const r=document.getElementById('ngv-desk'),d=r.querySelector('.dialog'),b=d.getBoundingClientRect();return {ready:!!r.ngvTest,width:b.width,height:b.height,overflow:d.scrollWidth>d.clientWidth,checkGaps:[...d.querySelectorAll('.finish-check')].map(l=>l.querySelector('span').getBoundingClientRect().left-l.querySelector('input').getBoundingClientRect().right),axes:[...d.querySelectorAll('h2,.studio-settings-tabs,.dialog-actions')].map(e=>({tag:e.tagName,left:e.getBoundingClientRect().left,right:e.getBoundingClientRect().right}))}})()")));
 const shot=await call('Page.captureScreenshot',{format:'png',captureBeyondViewport:false},topSession);await writeFile(here+'/review/settings-layout-2026-09-19/inline-general.png',Buffer.from(shot.data,'base64'));
