...
try{
 ...
 const {targetId}=await call('Target.createTarget',{url:'file:///tmp/ngv-artwork-host.html'});
 ...
 await evalJS("document.getElementById('ngv-desk').ngvTest.fixture('Export');document.getElementById('ngv-desk').ngvTest.action('finish-section:preview')");
 await sleep(400);
 const icons=await inspect('Array.from(document.querySelectorAll("svg[data-lucide]")).map(s=>({name:s.getAttribute("data-lucide"),svg:s.outerHTML}))');
 console.log(JSON.stringify(await evalJS("({ready:!!document.getElementById('ngv-desk').ngvTest,heading:document.getElementById('nd-surface-tools').textContent,icons:document.querySelectorAll('svg[data-lucide]').length,width:document.getElementById('ngv-desk').clientWidth,height:document.getElementById('ngv-desk').clientHeight})"))); 
 const shot=await call('Page.captureScreenshot',{format:'png',captureBeyondViewport:false},topSession);await writeFile('/tmp/ngv-artwork-host-review.png',Buffer.from(shot.data,'base64'));
