...
try{
 ...
 const {targetId}=await call('Target.createTarget',{url:'file:///tmp/ngv-toolbar-icons.html'});
 ...
 await evalJS("document.getElementById('ngv-desk').ngvTest.fixture('Postproduction');document.getElementById('ngv-desk').ngvTest.action('workspace:media')");
 await sleep(400);
 ...
 await writeFile('/tmp/ngv-toolbar-supplied-icons.json',JSON.stringify(icons));
 const shot=await call('Page.captureScreenshot',{format:'png',captureBeyondViewport:false},topSession);await writeFile('/tmp/ngv-toolbar-host-review.png',Buffer.from(shot.data,'base64'));
