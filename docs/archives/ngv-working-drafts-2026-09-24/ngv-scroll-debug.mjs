...
try {
 ...
 await call('Emulation.setDeviceMetricsOverride',{width:768,height:1000,deviceScaleFactor:1,mobile:false},session);
 await run("a.fixture('Storyboard');a.action('open:script')");
 ...
 const log=async label=>console.log(label,JSON.stringify(await run(`const c=q('#nd-canvas'),t=q('.shot-tile[aria-pressed=true]');return {scroll:c.scrollTop,height:c.clientHeight,total:c.scrollHeight,selected:st().selected,tile:t?.getBoundingClientRect().toJSON(),canvas:c.getBoundingClientRect().toJSON()}`)));
 await log('immediate');await sleep(100);await log('later');
 await run('a.render()');await sleep(100);await log('rerender');
 await run(`q('#nd-canvas').scrollTop=450`);await sleep(100);await log('explicit');
