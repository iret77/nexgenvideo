...
try {
 ...
 await test('Review follows the same montage',`const old=st().finish.scanKey;a.action('workspace:edit');a.action('studio:duplicate');a.action('workspace:post');a.action('finish:scan');return st().finish.scanKey!==old`);
 ...
 await test('Take review shows machine findings without mandatory checkboxes',`a.fixture('Takes');a.action('studio:take-review');return !q('.dialog input[type="checkbox"]')&&!q('[data-do="wf:take-select"]').disabled`);
 await click('[data-do="wf:take-select"]');
 await test('Take review records exact revision',`return st().studio.takeReviews['0-1'].revision===st().version&&st().chosen[0]===1`);
 await test('Blocking preserves shot',`a.fixture('Blocking');a.choose(3);a.action('studio:blockout');return st().view==='blocking'&&st().selected[0]===3&&q('canvas[data-clay="space"]')`);await shot('blocking');
