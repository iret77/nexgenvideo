...
try {
 ...
 await test('Legacy research task is hidden after restoring a project',`const s=st();s.studio.task={...s.studio.modelResearch,stage:'prepare'};s.studio.modelResearch=null;a.restore(s);return st().view==='takes'&&q('#nd-task').hidden&&!q('.dialog')`);
 ...
 await test('Settings research preserves the paused production order',`const id=st().studio.task.id;a.action('studio:settings');a.action('studio:model-research');a.action('studio:task-preview');a.action('studio:task-accept');a.action('studio:modal-close');return st().studio.task.id===id&&st().studio.task.stage==='paused'&&st().studio.task.completed>0`);
