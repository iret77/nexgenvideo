...
enum ToolDefinitions {
    ...
    private static let base: [AgentTool] = [
        ...
        AgentTool(
            ...
            description: "Apply the same property values to one or more clips in a single undoable action. Pass any combination of durationFrames, trimStartFrame, trimEndFrame, speed, volume, opacity, transform, crop, or — for text clips only — content, fontName, fontSize, color, alignment. All values are validated for every target before any clip changes. All values are applied to every clip in clipIds; for per-clip differences, make separate calls. trimStartFrame/trimEndFrame are offsets from the source media, not the timeline. speed 1.0 is normal, <1.0 slows (clip gets longer on the timeline), >1.0 speeds up. volume and opacity are 0.0–1.0. transform uses 0–1 normalized canvas coords, partial merge (pass only centerY to reposition vertically); flipHorizontal/flipVertical mirror the clip across the corresponding axis (no effect on text clips). crop is a partial set of 0–1 edge insets in the display-oriented source frame, origin top-left, before effects and transform. Omitted crop edges keep the effective animated crop at the current playhead, clamped separately to each target clip's first or last frame when the playhead is outside it; all zeros restore the full source. At least 5% must remain visible on each axis. Crop does not change the project/canvas aspect, transform box, position, rotation, or flips; its visible pixel aspect is sourceAspect × visibleWidth ÷ visibleHeight. A static crop clears crop keyframes; use set_keyframes for animated crop. When a text clip's content or font changes without an explicit transform, the bounding box auto-refits. Text-only fields with any non-text clip in clipIds are rejected; crop accepts only video, image, and Lottie clips.\n\nFor moves and start-frame changes, use move_clips. For animated values (keyframes), use set_keyframes — setting volume, opacity, or crop here clears any existing keyframe track on that property.\n\nTiming changes (durationFrames, trimStartFrame, trimEndFrame, speed) on a linked clip carry over to its linked partner so audio/video stay in sync — same as the timeline UI. Per-clip fields (volume, opacity, transform, crop, text*) don't propagate. trim and speed are skipped for text partners.",
            inputSchema: objectSchema(
                properties: [
                    "clipIds": [
                        "type": "array",
        ...
        AgentTool(
            ...
            description: "Write the human/agent interpretation onto the existing measured analysis artifact. Use this after run_phase(\"analysis\") and instead of editing analysis JSON. The host requires structure_resolution.status to be resolved or review_required with independently reproducible evidence for every boundary; labels can never hide needs_review timing. It preserves measured timing, boundary evidence, all analysis diagnostics, and detector anomalies, requires exactly one label for every canonical section, restricts the perceived-tempo multiplier to 0.5/1/2, mirrors labels onto the sections, and writes atomically.",
