...
enum ToolDefinitions {
    static let all: [AgentTool] = [
        ...
        AgentTool(
            ...
            description: "Write the human/agent interpretation onto the existing measured analysis artifact. Use this after run_phase(\"analysis\") and instead of editing analysis JSON. The host requires structure_resolution.status to be resolved from a complete Apple Music Understanding section/segment/phrase hierarchy; labels can never hide needs_review timing. It preserves measured timing, system-hierarchy evidence, all analysis diagnostics, and detector anomalies, requires exactly one label for every canonical section, restricts the perceived-tempo multiplier to 0.5/1/2, mirrors labels onto the sections, and writes atomically.",
