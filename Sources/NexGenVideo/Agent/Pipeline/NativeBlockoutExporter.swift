    ...
    private static func draw(
        ...
        for (index, setup) in ordered.enumerated() {
            ...
            let projected = points.map { point in
                CGPoint(
                    x: CGFloat(width) * 0.5 + CGFloat(point.x) * CGFloat(grid),
                    y: CGFloat(height) * 0.5 + CGFloat(point.z) * CGFloat(grid)
                )
            }
            if let first = projected.first {
                context.move(to: first)
                for point in projected.dropFirst() { context.addLine(to: point) }
                context.strokePath()
                let targetIndex = min(
                    projected.count - 1,
                    Int((Double(projected.count - 1) * progress).rounded())
                )
                let camera = projected[targetIndex]
                context.fillEllipse(in: CGRect(x: camera.x - 7, y: camera.y - 7, width: 14, height: 14))
            }
        }
    }
}
