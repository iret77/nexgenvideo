...
enum CIKernelLoader {
    private static func metallibURL(_ lib: String) -> URL? {
        ...
        let candidates = [
            ...
            Bundle.main.resourceURL?.appendingPathComponent("NexGenVideo_NexGenVideo.bundle/\(lib).metallib"), // swift run
            buildDir.appendingPathComponent("NexGenVideo_NexGenVideo.bundle/\(lib).metallib"),                 // swift test
        ].compactMap { $0 }
