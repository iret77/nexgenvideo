    ...
    func resume(editor: EditorViewModel, retryKnownJobs: Bool = true) {
        ...
        Task { @MainActor [weak self, weak editor] in
            do {
                ...
                let reconciled = try GenerationBatchStore.reconcileRuntime(
                    snapshots,
                    editor: editor
                )
                self.snapshots = reconciled
                self.error = nil
                let candidates = reconciled.filter { snapshot in snapshot.authorityAvailable && snapshot.journal.executions.contains(where: {
