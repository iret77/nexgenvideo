...
extension GenerationProvider {
    ...
    var mcpCapability: MCPCapability? {
        ...
        case .fal, .runway, .google, .elevenlabs, .marble:
            ...
            // Google serves its image models over plain REST on the user's own key (#212) and ships
            // no MCP server we reach. Showing an MCP field for any of these would be a dead/misleading
            // affordance — they use the API key.
            return nil
        }
    }
