...
class BpyRuntimeWorkflowTests(unittest.TestCase):
    def test_acceptance_is_manual_and_uses_existing_runner_split(self):
        ...
        self.assertIn('.prefixedUserGeometryState == "resourceLimited"', text)
    ...
    def test_worker_results_are_host_owned(self):
        ...
        self.assertIn("procPIDRegionPathInfo", service)
        self.assertIn("ownedRetainedVnodes", service)
        self.assertIn("kernelFRead: UInt32 = 1", service)
        self.assertIn("kernelFWrite: UInt32 = 2", service)
        self.assertIn("traversedRegions > 0", service)
        self.assertIn("resources[identity] = max(resources[identity] ?? 0, bytes)", service)
        self.assertNotIn("O_ACCMODE", service)
        self.assertNotIn("PROC_PIDFDVNODEINFO", service)
